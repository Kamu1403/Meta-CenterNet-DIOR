...
class MetaCenterNetHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 init_cfg=None):
        super(MetaCenterNetHead, self).__init__(init_cfg)
        ...
        if with_meta_cls_loss:
            # self.conv_meta = nn.Conv2d(meta_cls_in_channels, 1024, 7, 1, bias=False)
            self.conv_meta = nn.Sequential(
                nn.BatchNorm2d(meta_cls_in_channels),
                nn.AvgPool2d(7, 1)
            )
    ...
    def decode_heatmap(self,
        ...
        *batch_dets, topk_ys, topk_xs = get_topk_from_heatmap(
        ...
        # if wh.shape[-1] != 2:
        #     gather_ind = batch_topk_labels.unsqueeze(dim=2)
        #     gather_ind = torch.cat((gather_ind * 2, gather_ind * 2 + 1), dim=2)
        #     wh = wh.gather(2, gather_ind)
        #     offset = offset.gather(2, gather_ind)
    ...
    def forward_meta_cls(self, support_feat: Tensor) -> Tensor:
        ...
        """
        meta_cls_score = self.conv_meta(support_feat)
        meta_cls_score = self.fc_meta(meta_cls_score.squeeze().squeeze())
